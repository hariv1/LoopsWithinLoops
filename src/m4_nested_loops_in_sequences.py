...
def multiply_numbers(sequence_of_lists):
    ...
    for i in range(len(sequence_of_lists)):
        sub_list= sequence_of_lists[i]
        for k in range(len(sub_list)):
            sub_list[k] = sub_list[k] * (i+1)
...
def run_test_sum_numbers():
    ...
    # Test 3:
    expected = 100
    answer = sum_numbers(([10,12], [5], [18,20,5,30]))
    print('Expected and actual are:', expected, answer)
...
def print_characters(sequence_of_strings):
    ...
    for i in range(len(sequence_of_strings)):
        sub_sequence = sequence_of_strings[i]
        for k in range(len(sub_sequence)):
            print(sub_sequence[k])
